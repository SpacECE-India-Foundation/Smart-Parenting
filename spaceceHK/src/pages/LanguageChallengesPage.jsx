...
function ReadingFluency({ ageFilter }) {
  ...
  useEffect(() => {
    getFluencyPassages().then(fs => {
      ...
      const filtered = ageFilter === "All" ? merged : merged.filter(x => x.age_group === ageFilter || !x.age_group);
      setPassages(filtered.length ? filtered : merged);
      setLoading(false);
    });
  }, [ageFilter]);
  ...
  const passage = passages[idx % Math.max(passages.length, 1)];
  if (!passage) return <div className={s.actBox}><p>No passages for this age group.</p></div>;

  const start = () => {
    setRunning(true); setTime(passage.time_limit||60); setDone(false);
    timerRef.current = setInterval(() => {
      setTime(t => { if (t<=1) { clearInterval(timerRef.current); setRunning(false); setDone(true); return 0; } return t-1; });
    }, 1000);
  };

  const wordCount = passage.text?.trim().split(/\s+/).length || 0;
  const timeLimit = passage.time_limit || 60;
  ...
  return (
    <div className={s.actBox}>
      ...
      <p className={s.actSub}>Read the passage aloud as fast and clearly as you can!</p>
      <div className={s.passCard}>
        <div className={s.passHead}>
          <strong>{passage.title}</strong>
          <span className="tag tag-diff">{passage.level}</span>
        </div>
        <p className={s.passText}>{passage.text}</p>
      </div>
      {!running && !done && (
      ...
      {done && (
        <div className={s.fluencyResult}>
          ...
          <div style={{display:"flex",gap:10,marginTop:12,flexWrap:"wrap"}}>
            ...
            <button className="btn btn-ghost" onClick={() => { setIdx(i=>(i+1)%passages.length); setDone(false); }}>Next Passage →</button>
          </div>
        </div>
      )}
    </div>
