// src/pages/admin/AdminScores.jsx — Per-activity score tracking with Edit, Delete & Username display
import { useState, useEffect } from "react";
import { getAllScores, updateScore, deleteScore, getStories } from "../../firebase/firestoreService";
import { storiesData } from "../../data/storiesData";
import s from "./Admin.module.css";

function Toast({ msg }) { return msg ? <div className={s.toast}>{msg}</div> : null; }

const ACTIVITY_TYPES = ["All", "story", "picture_match", "word_builder", "flashcard", "sound_match", "phonics", "fluency"];
const TYPE_LABELS = {
  story:         "📖 Story Quiz",
...
const TYPE_COLORS = {
  story:         "badgeGreen",
  picture_match: "badgeOrange",
  word_builder:  "badgeBlue",
  flashcard:     "badgePurple",
  sound_match:   "badgePink",
  phonics:       "badgeYellow",
  fluency:       "badgeTeal",
};
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:999,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      <div style={{background:"var(--color-surface)",borderRadius:16,padding:24,width:"100%",maxWidth:440,boxShadow:"0 20px 60px rgba(0,0,0,0.3)"}}>
        ...
        <div className="form-group">
          ...
          <select className="form-input" value={form.activity_type} onChange={e=>setF("activity_type",e.target.value)}>
        ...
        <div className="form-group">
          ...
          <input className="form-input" value={form.activity_id} onChange={e=>setF("activity_id",e.target.value)} placeholder="story id or activity id" />
        </div>
...
export default function AdminScores() {
  ...
  // Helper to get story info from both sources
  ...
  const handleSaveEdit = async (form) => {
    setBusy(true);
    await updateScore(editItem.id, editItem._col || "language_scores", form);
  ...
  const typeMap = {};
  scores.forEach(sc => {
    const type = sc.activity_type || "story";
    if (!typeMap[type]) typeMap[type] = { attempts:0, correct:0, total_score:0 };
    typeMap[type].attempts++;
    typeMap[type].total_score += (sc.score||0);
    if (sc.accuracy===1) typeMap[type].correct++;
  });
  const typeRows = Object.entries(typeMap).map(([type,d])=>({
    type,
  ...
  // PER-STORY breakdown — now includes per-user breakdown
  const storyScores = scores.filter(sc=>(sc.activity_type||"story")==="story");
  const storyMap = {};
  storyScores.forEach(sc => {
    ...
    if (!storyMap[id].users[uid]) storyMap[id].users[uid] = { attempts:0, correct:0, total_score:0, username: sc.username || sc.display_name || uid };
  ...
  const filtered = scores.filter(sc=>{
    const username = sc.username || sc.display_name || "";
    const matchSearch = (sc.child_id||"").toLowerCase().includes(search.toLowerCase()) ||
                        (sc.activity_id||"").toLowerCase().includes(search.toLowerCase()) ||
                        username.toLowerCase().includes(search.toLowerCase());
    const matchType   = typeFilter==="All" || (sc.activity_type||"story")===typeFilter;
  ...
  return (
    <>
      ...
      <div style={{display:"flex",gap:10,marginBottom:24,flexWrap:"wrap"}}>
        {[
          ["table","📋 All Scores"],
          ["bytype","📊 By Activity Type"],
          ["perstory","📚 Per Story + Users"],
        ].map(([id,label])=>(
          <button key={id} className={`${s.chipBtn} ${view===id?s.chipActive:""}`} onClick={()=>setView(id)}>
            {label}
          </button>
        ))}
      </div>
      ...
      {view==="bytype" && (
        <div className={s.section}>
          <h2 className={s.sectionTitle} style={{marginBottom:16}}>📊 Score Breakdown by Activity Type</h2>
          {typeRows.length===0
            ? <p className={s.empty}>No scores yet.</p>
            : <div className={s.tableWrap} style={{overflowX:"auto"}}>
                <table className={s.table}>
                  <thead>
                    <tr><th>Activity</th><th>Attempts</th><th>Correct</th><th>Avg Score</th><th>Accuracy</th></tr>
                  </thead>
                  <tbody>
                    {typeRows.map(row=>(
                      <tr key={row.type}>
      ...
      {/* PER STORY VIEW — with expandable user breakdown */}
      {view==="perstory" && (
        <div className={s.section}>
          ...
          <p style={{fontSize:13,color:"var(--color-text-secondary)",marginBottom:16,fontWeight:600}}>
            Click on a story row to see per-user scores 👆
          </p>
          {storyRows.length===0
                        ...
                        {expandedStory===row.id && row.users.map(u=>(
                          <tr key={u.uid} style={{background:"var(--color-bg-elevated)"}}>
                            <td style={{paddingLeft:32}}>
                              ...
                              <strong style={{marginLeft:6,fontSize:13}}>
                                {u.username !== u.uid ? u.username : u.uid.slice(0,12)+"…"}
                              </strong>
                              {u.username !== u.uid && (
      ...
      {view==="table" && (
        <div className={s.section}>
          <div className={s.sectionHead}>
            <h2 className={s.sectionTitle}>📋 All Score Records ({filtered.length})</h2>
            <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"center"}}>
              <input className="form-input" style={{width:"100%",maxWidth:220}}
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:12}}>
            <span style={{fontSize:13,fontWeight:600,alignSelf:"center"}}>Type:</span>
            {ACTIVITY_TYPES.map(f=>(
              <button key={f} className={`${s.chipBtn} ${typeFilter===f?s.chipActive:""}`}
          ...
          <div className={s.tableWrap} style={{overflowX:"auto"}}>
            <table className={s.table}>
              <thead>
                <tr><th>User</th><th>Story / Activity</th><th>Type</th><th>Score</th><th>Result</th><th>Date</th><th>Actions</th></tr>
              </thead>
              <tbody>
                {loading
                  ? <tr className={s.loadingRow}><td colSpan={7}>Loading scores...</td></tr>
                  : filtered.length===0
                  ? <tr><td colSpan={7} className={s.empty}>
                      {scores.length===0
                        ? "No scores yet. They appear after users complete activities!"
                        : "No results match your filter."}
                    </td></tr>
                  : filtered.map(sc=>{
                      const story = getStoryInfo(sc.activity_id);
                      const typeKey = sc.activity_type || "story";
                      ...
                      return (
                        <tr key={sc.id}>
                          <td>
                            {displayName ? (
                              <div>
                                ...
                                <div style={{fontFamily:"monospace",fontSize:10,color:"var(--color-text-secondary)",marginTop:2}}>
                                  {uid.slice(0,12)}…
                                </div>
                              </div>
                            ) : (
                          ...
                          <td>
                            <span className={`${s.badge} ${s[TYPE_COLORS[typeKey]]||s.badgeBlue}`}>
