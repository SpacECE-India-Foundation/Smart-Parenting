...
export default function AdminStories() {
  ...
  const load = async () => {
    ...
    setStories(fs.length ? fs : storiesData);
    setLoading(false);
  };
  useEffect(()=>{ load(); },[]);
  ...
  const handleDelete = async id => {
    ...
    setBusy(true);
    await deleteStory(id); showToast("🗑️ Story deleted!"); await load();
    setBusy(false);
  };
                    ...
                    return (
                      <tr key={st.id}>
                        ...
                        <td style={{whiteSpace:"nowrap"}}>
                          ...
                          <button className={`${s.iconBtn} ${s.deleteBtn}`} onClick={()=>handleDelete(st.id)} title="Delete" disabled={busy}>🗑️</button>
                        </td>
                      </tr>
                    );
                  })
