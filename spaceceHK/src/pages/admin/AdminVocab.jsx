...
function FlashcardsAdmin() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editId) {
        // editId set hai — always update (never add)
  ...
  const startEdit = it => {
    if (DEFAULT_VOCAB_IDS.has(it.id)) {
      showToast("⚠️ Default cards seedha edit nahi ho sakte — pehle apna card add karo!");
      return;
    }
    setEditId(it.id);
  ...
  const handleDelete = async id => {
    if (DEFAULT_VOCAB_IDS.has(id)) { showToast("⚠️ Default vocab delete nahi ho sakta."); return; }
...
function WordBuilderAdmin() {
  ...
  const startEdit = it => {
    if (DEFAULT_WB_IDS.has(it.id)) {
      showToast("⚠️ Default words seedha edit nahi ho sakte!");
      return;
    }
    setEditId(it.id);
  ...
  const handleDelete = async id => {
    if (DEFAULT_WB_IDS.has(id)) { showToast("⚠️ Default words delete nahi ho sakte."); return; }
...
function PictureMatchAdmin() {
  ...
  const startEdit = it => {
    if (DEFAULT_PM_IDS.has(it.id)) {
      showToast("⚠️ Default items seedha edit nahi ho sakte!");
      return;
    }
    setEditId(it.id);
  ...
  const handleDelete = async id => {
    if (DEFAULT_PM_IDS.has(id)) { showToast("⚠️ Default items delete nahi ho sakte."); return; }
